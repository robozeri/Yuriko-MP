...
use pocketmine\event\player\PlayerBowAimEvent;
...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	public function setSkin($str, $skinName = false){
	...
	public function onUpdate($currentTick){
		...
		if($this->spawned){
			...
			if($this->isSurvival() or $this->isAdventure()){
				...
				if($this->foodTick >= 80){
					if($this->getHealth() < $this->getMaxHealth() and $this->getFood() >= 18) {
	...
	protected function processLogin(){
		...
		$this->getAttributeManager()->getAttribute(AttributeManager::MAX_HEALTH)->setValue($this->getHealth());
		$this->getAttributeManager()->getAttribute(AttributeManager::MAX_HUNGER)->setValue($this->getFood());
		$this->getAttributeManager()->sendAll();
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::USE_ITEM_PACKET:
				...
				if($packet->face >= 0 and $packet->face <= 5){ //Use Block, place
					...
					if($item->getId() === Item::SNOWBALL){
						...
						$this->server->getPluginManager()->callEvent(new PlayerBowAimEvent($this));
					}
			...
			case ProtocolInfo::PLAYER_ACTION_PACKET:
				...
				switch($packet->action){
					...
					case PlayerActionPacket::ACTION_RESPAWN:
						...
						$this->setFood(20);
						$this->getAttributeManager()->resetAll();
						$this->starvationTick = 0;
						$this->foodTick = 0;
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()){
					...
					$damage = [
						EntityDamageEvent::MODIFIER_BASE => $item instanceof Tool ? $item->getDamageTable() : 1,
					];
					...
					if(!$this->canInteract($target, 8)){
						...
						$points = 0;
						foreach($target->getInventory()->getArmorContents() as $i){
